...
async function triggerAutomations(triggerType, eventPayload) {
  try {
    const automations = getDb().prepare(`
...
async function dispatchWebhooks(eventType, payload) {
  const activeWebhooks = getDb().prepare(`
...
async function processRetryTask(taskId) {
  const task = getDb().prepare('SELECT * FROM retry_queue WHERE id = ?').get(taskId);
  if (!task) throw new Error('Retry task not found');

  getDb().prepare("UPDATE retry_queue SET status = 'processing', updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(taskId);
  ...
  try {
    ...
    getDb().prepare("UPDATE retry_queue SET status = 'completed', last_error = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(taskId);
...
async function retryAllPendingTasks() {
  const pending = getDb().prepare("SELECT id FROM retry_queue WHERE status = 'pending' OR status = 'failed' LIMIT 50").all();
  let successCount = 0;
  let failCount = 0;
