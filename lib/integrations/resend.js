...
function getResendConfig() {
  const row = db.prepare("SELECT * FROM integrations WHERE provider = 'resend'").get();
  let dbConfig = {};
  if (row) {
...
async function sendEmail({ to, subject, html, text, slug, variables = {} }) {
  ...
  if (slug) {
    const tpl = db.prepare('SELECT * FROM email_templates WHERE slug = ?').get(slug);
    if (tpl) {
      if (tpl.is_enabled === 0) {
  ...
  if (!apiKey) {
    ...
    logActivity(null, `Email Dispatched (Simulated): ${emailSubject}`, 'Email', slug || 'custom', '127.0.0.1', 'Resend', `To: ${to}`);
  ...
  try {
    ...
    if (response.ok && data.id) {
      ...
      logActivity(null, `Email Delivered via Resend: ${emailSubject}`, 'Email', slug || 'custom', '127.0.0.1', 'Resend', `ID: ${data.id} To: ${to}`);
