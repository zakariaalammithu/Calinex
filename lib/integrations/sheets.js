...
function getSheetsConfig() {
  const row = db.prepare("SELECT * FROM integrations WHERE provider = 'google_sheets'").get();
...
async function syncLeadToSheets(messageId, leadData) {
  ...
  if (config.auto_sync === false) {
    db.prepare("UPDATE messages SET sheets_sync_status = 'none' WHERE id = ?").run(messageId);
  ...
  if (!webhookUrl && !config.access_token) {
    ...
    db.prepare("UPDATE messages SET sheets_sync_status = 'synced' WHERE id = ?").run(messageId);
    db.prepare(`
    ...
    logActivity(null, `Synced Lead #${messageId} to Google Sheets (Simulated)`, 'GoogleSheets', messageId.toString(), '127.0.0.1', 'SheetsSync');
  ...
  try {
    ...
    if (response.ok) {
      ...
      logActivity(null, `Synced Lead #${messageId} to Google Sheets Live`, 'GoogleSheets', messageId.toString(), '127.0.0.1', 'SheetsSync');
      return { success: true };
    } else {
    ...
    db.prepare("UPDATE messages SET sheets_sync_status = 'pending' WHERE id = ?").run(messageId);
    db.prepare(`
      UPDATE integrations SET
        status = 'error',
...
async function manualSyncPendingLeads() {
  const pendingMessages = db.prepare("SELECT * FROM messages WHERE sheets_sync_status = 'pending' ORDER BY id ASC LIMIT 50").all();
  let synced = 0;
  let failed = 0;
