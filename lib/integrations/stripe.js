...
function getStripeConfig() {
  const row = getDb().prepare("SELECT * FROM integrations WHERE provider = 'stripe'").get();
  if (!row) {
    return {
      status: 'disconnected',
      config: {
...
async function createCheckoutSession({ clientName, clientEmail, serviceName, amount, currency = 'USD', successUrl, cancelUrl }) {
  ...
  getLogActivity()(null, `Payment Created: $${numAmount} ${currency} for ${clientName}`, 'Payment', resDb.lastInsertRowid.toString(), '127.0.0.1', 'Stripe');
...
async function processRefund(paymentId, refundAmount = null) {
  ...
  getLogActivity()(null, `Refund Processed for Payment #${paymentId} ($${amountToRefund})`, 'Payment', paymentId.toString(), '127.0.0.1', 'Stripe');
...
async function handleStripeWebhook(rawPayload, signatureHeader) {
  ...
  if (eventType === 'checkout.session.completed' || eventType === 'payment_intent.succeeded') {
    ...
    const existing = getDb().prepare('SELECT id FROM payments WHERE stripe_checkout_session_id = ? OR stripe_payment_intent_id = ?').get(sessionId, paymentIntentId);
    ...
    getLogActivity()(null, `Stripe Webhook Processed (${eventType}): $${amountTotal} ${currency}`, 'Stripe', paymentRecordId.toString(), '127.0.0.1', 'StripeWebhook');
