...
function getCalcomConfig() {
  const row = getDb().prepare("SELECT * FROM integrations WHERE provider = 'cal_com'").get();
...
async function processCalcomWebhook(payload = {}) {
  try {
    ...
    if (!clientPhone && clientEmail) {
      const matchContact = getDb().prepare('SELECT phone FROM contacts WHERE email = ? LIMIT 1').get(clientEmail);
      if (matchContact && matchContact.phone) clientPhone = matchContact.phone;
      else {
        const matchMsg = getDb().prepare('SELECT phone FROM messages WHERE email = ? AND phone IS NOT NULL AND phone != "" LIMIT 1').get(clientEmail);
    ...
    const existing = getDb().prepare('SELECT id FROM bookings WHERE cal_booking_id = ?').get(calBookingId);
    let bookingId = null;
    ...
    getLogActivity()(null, `Cal.com Webhook Processed (${triggerEvent})`, 'CalCom', bookingId.toString(), '127.0.0.1', 'CalComWebhook', `Client: ${clientName}`);
