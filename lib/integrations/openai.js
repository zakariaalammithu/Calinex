...
function getOpenAIConfig() {
  const row = getDb().prepare("SELECT * FROM integrations WHERE provider = 'openai'").get();
  if (!row) {
    return {
      status: 'disconnected',
      config: {
...
async function analyzeLead(leadData, messageId = null) {
  ...
  if (messageId) {
    ...
    getLogActivity()(null, `AI Lead Analysis Generated (Score: ${analysisResult.score})`, 'OpenAI', messageId.toString(), '127.0.0.1', 'OpenAI');
  }
