...
async function handlePublicApi(req, res) {
  ...
  if ((pathname === '/api/public/case-studies' || pathname === '/api/case-studies') && req.method === 'GET') {
    const fs = require('fs');
    const path = require('path');
    const csFile = process.env.VERCEL
      ? path.join('/tmp', 'case-studies.json')
      : path.join(__dirname, '..', 'data', 'case-studies.json');
    let caseStudies = [];
    try {
      if (fs.existsSync(csFile)) {
        caseStudies = JSON.parse(fs.readFileSync(csFile, 'utf8'));
      } else {
        const defaultFile = path.join(__dirname, '..', 'data', 'case-studies.json');
        if (fs.existsSync(defaultFile)) {
          caseStudies = JSON.parse(fs.readFileSync(defaultFile, 'utf8'));
        }
      }
    } catch (e) {
      console.error('[Public API] Error loading case studies:', e);
    }
    ...
    return res.end(JSON.stringify({ success: true, caseStudies: published }));
  }
