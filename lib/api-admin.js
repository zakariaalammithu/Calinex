...
async function handleAdminApi(req, res) {
  ...
  if (pathname === '/api/admin/dashboard/stats' && method === 'GET') {
    ...
    const newContacts = db.prepare("SELECT COUNT(*) as c FROM contacts WHERE status = 'lead'").get().c;
    ...
    const activeAutomations = db.prepare("SELECT COUNT(*) as c FROM automations WHERE is_enabled = 1").get().c;
    const activeIntegrations = db.prepare("SELECT COUNT(*) as c FROM integrations WHERE status = 'connected'").get().c;
    ...
    const recentLeads = db.prepare(`
      SELECT id, name, email, company, service, budget, ai_score, ai_intent, status, created_at
      FROM messages
    ...
    const recentBookings = db.prepare(`
      SELECT id, client_name, client_email, meeting_type, booking_date, time_slot, status
      FROM bookings
      ORDER BY created_at DESC
      LIMIT 5
    `).all();
