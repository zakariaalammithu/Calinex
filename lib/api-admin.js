...
async function handleAdminApi(req, res) {
  ...
  if ((pathname === '/api/admin/auth/login' || pathname === '/api/admin/login') && method === 'POST') {
    ...
    try {
      ...
      console.warn('[ADMIN AUTH DB NOTICE] Retrying query after schema init:', err.message);
      try {
        ...
        console.error('[ADMIN AUTH ERROR] Failed to fetch user record:', retryErr.message);
      }
    }
...
function validateIntegrationCredentials(provider, cfg) {
  ...
  const CASE_STUDIES_FILE = process.env.VERCEL
    ? path.join('/tmp', 'case-studies.json')
    : path.join(__dirname, '..', 'data', 'case-studies.json');

  function getCaseStudiesStore() {
    try {
      if (fs.existsSync(CASE_STUDIES_FILE)) {
        return JSON.parse(fs.readFileSync(CASE_STUDIES_FILE, 'utf8'));
      }
      const defaultFile = path.join(__dirname, '..', 'data', 'case-studies.json');
      if (fs.existsSync(defaultFile)) {
        const data = JSON.parse(fs.readFileSync(defaultFile, 'utf8'));
        if (process.env.VERCEL) {
          try { fs.writeFileSync(CASE_STUDIES_FILE, JSON.stringify(data, null, 2), 'utf8'); } catch(e){}
        }
        return data;
      }
    } catch (e) {
      console.error('[CMS Error] Error reading case-studies.json:', e);
    }
    return [];
  }

  function saveCaseStudiesStore(studies) {
    try {
      const dataDir = path.dirname(CASE_STUDIES_FILE);
      if (!fs.existsSync(dataDir)) {
        fs.mkdirSync(dataDir, { recursive: true });
      }
      fs.writeFileSync(CASE_STUDIES_FILE, JSON.stringify(studies, null, 2), 'utf8');
      if (!process.env.VERCEL) {
        const mainPath = path.join(__dirname, '..', 'data', 'case-studies.json');
        fs.writeFileSync(mainPath, JSON.stringify(studies, null, 2), 'utf8');
      }
      return true;
    } catch (e) {
      console.error('[CMS Error] Error saving case-studies.json:', e);
      return false;
    }
  }

  if (pathname === '/api/admin/cms/case-studies' && method === 'GET') {
    const caseStudies = getCaseStudiesStore();
    caseStudies.sort((a, b) => (Number(a.order) || 999) - (Number(b.order) || 999));
    return sendJson(200, { success: true, caseStudies });
  }

  if ((pathname === '/api/admin/cms/case-studies' || pathname === '/api/admin/cms/case-studies/save') && method === 'POST') {
    const body = await parseBody(req);
    if (!body || !body.title) {
      return sendJson(400, { success: false, error: 'Project Title is required' });
    }

    const studies = getCaseStudiesStore();
    const slug = (body.slug || body.id || body.title.toLowerCase().replace(/[^a-z0-9]+/g, '-')).replace(/^\/+|\/+$/g, '');
    const existingIdx = studies.findIndex(s => s.id === slug || s.slug === slug || (body.id && s.id === body.id));
    ...
    const updatedItem = {
      ...
      fullDescription: (body.fullDescription || body.description || '').trim(),
      client: (body.client || body.title).trim(),
      services: (body.services || '').trim(),
      location: (body.location || 'United States').trim(),
      date: (body.date || '2025').trim(),
      metricNumber: (body.metricNumber || '').trim(),
      metricLabel: (body.metricLabel || '').trim(),
      image: body.image || 'https://cdn.prod.website-files.com/6655d16113e6966ef4eb1054/6a3b69488d9cfd0f31763daf_Kodezi.png',
      srcset: body.srcset || '',
      gallery: Array.isArray(body.gallery) ? body.gallery : (typeof body.gallery === 'string' ? body.gallery.split(',').map(g => g.trim()).filter(Boolean) : (body.image ? [body.image] : [])),
      tags: Array.isArray(body.tags) ? body.tags : (typeof body.tags === 'string' ? body.tags.split(',').map(t => t.trim()).filter(Boolean) : ['Websites']),
      link: `/case-studies.html/${slug}`,
      status: body.status === 'unpublished' ? 'unpublished' : 'published',
      featured: body.featured === true || body.featured === 'true',
      order: Number(body.order) || (existingIdx >= 0 ? studies[existingIdx].order : studies.length + 1),
      createdAt: existingIdx >= 0 ? studies[existingIdx].createdAt : new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };

    if (existingIdx >= 0) {
      studies[existingIdx] = updatedItem;
    } else {
      studies.push(updatedItem);
    }

    studies.sort((a, b) => (Number(a.order) || 999) - (Number(b.order) || 999));
    saveCaseStudiesStore(studies);

    logActivity(currentUser.id, `Saved Case Study "${updatedItem.title}" (${updatedItem.slug})`, 'CMS', updatedItem.slug, getClientIp(req), req.headers['user-agent']);
    return sendJson(200, { success: true, message: 'Case Study saved successfully', caseStudy: updatedItem });
  }

  if ((pathname === '/api/admin/cms/case-studies/delete' || pathname.startsWith('/api/admin/cms/case-studies/delete/')) && (method === 'POST' || method === 'DELETE')) {
    const body = await parseBody(req);
    const idToDelete = body.id || body.slug || pathname.split('/').pop();
    if (!idToDelete) {
      return sendJson(400, { success: false, error: 'Case Study ID is required for deletion' });
    }

    let studies = getCaseStudiesStore();
    const initialLen = studies.length;
    studies = studies.filter(s => s.id !== idToDelete && s.slug !== idToDelete);

    if (studies.length === initialLen) {
      return sendJson(404, { success: false, error: 'Case Study not found' });
    }

    saveCaseStudiesStore(studies);
    logActivity(currentUser.id, `Deleted Case Study (${idToDelete})`, 'CMS', idToDelete, getClientIp(req), req.headers['user-agent']);
    return sendJson(200, { success: true, message: 'Case Study deleted successfully' });
  }

  if (pathname === '/api/admin/cms/case-studies/reorder' && method === 'POST') {
    const body = await parseBody(req);
    const items = body.items || body.caseStudies;
    if (!Array.isArray(items)) {
      return sendJson(400, { success: false, error: 'Array of items is required' });
    }

    const studies = getCaseStudiesStore();
    items.forEach(item => {
      const target = studies.find(s => s.id === item.id || s.slug === item.id);
      if (target && item.order !== undefined) {
        target.order = Number(item.order);
      }
    });
    ...
    logActivity(currentUser.id, 'Reordered Case Studies', 'CMS', null, getClientIp(req), req.headers['user-agent']);
    return sendJson(200, { success: true, message: 'Case Studies reordered successfully', caseStudies: studies });
  }

  if (pathname === '/api/admin/cms/upload' && method === 'POST') {
    const body = await parseBody(req);
    const { dataUrl, base64Data, filename } = body;
    let fileBuffer;
    let ext = '.png';
    ...
    if (fileBuffer) {
      const cleanName = `cs_${Date.now()}_${Math.random().toString(36).substring(2, 7)}${ext}`;
      const targetFile = path.join(UPLOADS_DIR, cleanName);
      try {
        fs.writeFileSync(targetFile, fileBuffer);
        const staticUploads = path.join(__dirname, '..', 'uploads');
        if (!process.env.VERCEL) {
          if (!fs.existsSync(staticUploads)) fs.mkdirSync(staticUploads, { recursive: true });
          fs.writeFileSync(path.join(staticUploads, cleanName), fileBuffer);
        }
        return sendJson(200, { success: true, url: `/uploads/${cleanName}` });
      } catch (e) {
        console.warn('[Upload Notice] Writing upload to disk failed, returning base64 Data URL:', e.message);
        return sendJson(200, { success: true, url: dataUrl });
      }
    }

    return sendJson(400, { success: false, error: 'Invalid or missing image payload' });
  }
